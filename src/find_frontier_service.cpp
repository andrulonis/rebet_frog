...
using GetFurthestFrontier = wavefront_frontier_msgs::srv::GetFurthestFrontier;

class FindFrontier : public RosServiceNode<GetFurthestFrontier>
{
public:
    ...
    FindFrontier(const std::string & instance_name,
        ...
        RosServiceNode<GetFurthestFrontier>(instance_name, conf, params)
    ...
    bool setRequest(typename Request::SharedPtr& request) override
    {
        ...
        RCLCPP_INFO(logger(), "Find frontier request");
        ...
        while(!(odom_res && map_res))
        {
            ...
            RCLCPP_INFO(logger(), "Blocking untl receiving both map and odom");
            RCLCPP_INFO(logger(), "Elapsed time: %f seconds", (now() - begin_wait_).seconds());
            if((now() - begin_wait_) > timeout )
            {
                RCLCPP_INFO(logger(), "Timed out waiting for both map and odom to be ready");
    ...
    {
        RCLCPP_INFO(logger(), "Response received find frontier");
...
CreateRosNodePlugin(FindFrontier, "findFrontier");
